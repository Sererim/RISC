...
export class SystemInterface implements MMIODevice{
    ...
    read(address: number) {
        ...
        if ((address & 0b11) !== 0) {
            throw new Error(`Unaligened read from address 0x${toHexString(address)}.`);
        }
    ...
    write(address: number, value: number) {
        if ((address & 0b11) !== 0) {
            throw new Error(`Unaligened write from address 0x${toHexString(address)} (value=0x${toHexString(value)}).`);
        }
